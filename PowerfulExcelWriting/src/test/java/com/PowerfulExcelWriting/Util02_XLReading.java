...
public class Util02_XLReading {


    public static String mGetExactCellType(XSSFCell oCell){
        String sExactCellType = null;
        switch (oCell.getCellType()){
            case NUMERIC:
                try {
                    Integer.parseInt(oCell.toString());
                    sExactCellType= "integer"; // long
                }catch (NumberFormatException ex){
                    sExactCellType= "float";   // double
                }
                break;
            case STRING:
                sExactCellType = "string";
                break;
            case BOOLEAN:
                sExactCellType = "boolean";
                break;
            case BLANK:
                sExactCellType = "blank";
                break;
            default:
                sExactCellType = "out of my league";
                break;
        }
        return sExactCellType;
    }
    ...
    public static XSSFCell mExcel_SetCellValue(String sExcelFile, String sSheetName, int nRowNum, int nColNum, Object objCellValue){
        ...
        try {
            ...
            if(    (objCellValue instanceof String) && ( ! objCellValue.toString().isEmpty())   ){
                ...
                CellStyle style1 = oRequiredCellValue.getCellStyle();
...
CellUtil.setAlignment(oRequiredCellValue,HorizontalAlignment.CENTER_SELECTION);
CellUtil.setVerticalAlignment(oRequiredCellValue,VerticalAlignment.CENTER);
        //properties.put(CellUtil.ALIGNMENT , CellStyle);
...
//
//        //CellUtil
//        CellUtil.setCellStyleProperties(oRequiredCellValue, properties);
